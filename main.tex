...
\begin{itemize}
\item Sensorów (w tym LiDAR - Light Detection and Ranging - Detekcja i Pomiar Światła)
\item Algorytmów SLAM (Simultaneous Localization and Mapping - Jednoczesna Lokalizacja i Mapowanie)
\item Szkieletów programistycznych dla robotów jak ROS (Robot Operating System - System Operacyjny Robotów)
\item Systemów nawigacji jak Nav2 (Navigation 2 - Nawigacja 2)
...
Prace nad projektem zostały podzielone na kilka kluczowych etapów. Rozpoczęto od analizy literatury i dostępnych rozwiązań, co pozwoliło na określenie optymalnego kierunku rozwoju systemu. Następnie przeprowadzono projekt i budowę platformy sprzętowej, która bazuje na dwóch współpracujących mikrokontrolerach - Raspberry Pi 4 oraz Arduino Nano, wyposażonych w czujnik RPLidar A1 oraz silniki DC z enkoderami. W kolejnym etapie zrealizowano implementację oprogramowania, obejmującą sterowniki urządzeń, algorytmy SLAM oraz system nawigacji. Po integracji wszystkich komponentów przeprowadzono szereg testów i optymalizacji, mających na celu zapewnienie stabilnej nawigacji i efektywnego mapowania otoczenia.
...
Problem jednoczesnej lokalizacji i mapowania (SLAM) oraz autonomicznej nawigacji robotów mobilnych stanowi jeden z kluczowych obszarów badań w dziedzinie robotyki. W ostatnich latach obserwuje się znaczący postęp w tej dziedzinie, głównie dzięki rozwojowi wydajnych algorytmów optymalizacji, poprawie jakości i dostępności sensorów (jak LiDAR czy kamery RGB-D), wzrostowi mocy obliczeniowej komputerów oraz powstaniu zaawansowanych frameworków robotycznych jak ROS 2.
...
SLAM Toolbox to zaawansowane narzędzie open-source dla ROS 2 implementujące algorytm Graph SLAM \cite{bib:slamtoolbox}. Algorytm wykorzystuje podejście oparte na grafie, gdzie węzły reprezentują pozycje robota, a krawędzie ograniczenia między tymi pozycjami wynikające z pomiarów. W czasie rzeczywistym tworzone są mapy 2D, przy czym system wykonuje ciągłą optymalizację grafu pozycji robota.

Proces SLAM rozpoczyna się od akwizycji skanów laserowych, które są następnie dopasowywane do istniejącej mapy metodą ICP (Iterative Closest Point). Algorytm konstruuje graf pozycji robota, który jest okresowo optymalizowany w celu minimalizacji błędów. Na podstawie zoptymalizowanego grafu aktualizowana jest mapa globalna, co zapewnia spójną reprezentację otoczenia.

\subsection{Navigation2 (Nav2)}
Nav2 to kompleksowy framework nawigacyjny dla ROS 2 \cite{bib:nav2}, który implementuje wielopoziomowe podejście do nawigacji autonomicznej. System wykorzystuje algorytm Dijkstry lub A* do globalnego planowania ścieżki, natomiast do lokalnego planowania ruchu stosuje algorytmy DWA (Dynamic Window Approach) lub TEB (Timed Elastic Band). Kontrola ruchu robota realizowana jest przez regulatory PID z uwzględnieniem ograniczeń kinematycznych platformy.
